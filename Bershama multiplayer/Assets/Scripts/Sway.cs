...
public class Sway : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        sway = Vector2.MoveTowards(sway, Vector2.zero, swayCurve.Evaluate(Time.fixedDeltaTime * swaySmoothCounteraction * sway.magnitude * swaySmooth));
        sway = Vector2.ClampMagnitude(new Vector2(mouseX, mouseY) + sway, maxSwayAmount);

        weaponTransform.localPosition = Vector3.Lerp(weaponTransform.localPosition, new Vector3(sway.x, sway.y, 0) * positionSwayMultiplier * Mathf.Deg2Rad + initialPosition, swayCurve.Evaluate(Time.fixedDeltaTime * swaySmooth));
        weaponTransform.localRotation = Quaternion.Slerp(weaponTransform.localRotation, initialRotation * Quaternion.Euler(Mathf.Rad2Deg * rotationSwayMultiplier * new Vector3(-sway.y, sway.x, 0)), swayCurve.Evaluate(Time.fixedDeltaTime * swaySmooth));
    }
